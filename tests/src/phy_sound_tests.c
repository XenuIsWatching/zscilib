...
ZTEST(zsl_tests, test_phy_sound_press_amp)
{
	...
	rc = zsl_phy_sound_press_amp(ZSL_CONSTANT(2.15E9), ZSL_CONSTANT(6.1E-4), ZSL_CONSTANT(5.1E-3), &p);
	zassert_true(rc == 0, NULL);
#ifdef CONFIG_ZSL_SINGLE_PRECISION
	zassert_true(val_is_equal(p, 6688.65, ZSL_CONSTANT(1E-3)), NULL);
#else
	zassert_true(val_is_equal(p, 6688.65, ZSL_CONSTANT(1E-8)), NULL);
#endif
...
ZTEST(zsl_tests, test_phy_sound_level)
{
	...
	rc = zsl_phy_sound_level(ZSL_CONSTANT(1.7E5), ZSL_CONSTANT(3.2E2), &b);
	zassert_true(rc == 0, NULL);
	zassert_true(val_is_equal(b, 27.2529894306, ZSL_CONSTANT(1E-6)), NULL);
...
ZTEST(zsl_tests, test_phy_sound_intensity)
{
	...
	rc = zsl_phy_sound_intensity(ZSL_CONSTANT(2.15E9), 997.6, 7851.5, &i);
	zassert_true(rc == 0, NULL);
	zassert_true(val_is_equal(i, 21.0464001945, ZSL_CONSTANT(1E-6)), NULL);
	...
	rc = zsl_phy_sound_intensity(ZSL_CONSTANT(2.15E9), 0.0, 7851.5, &i);
	zassert_true(rc == -EINVAL, NULL);
	/* IEEE standard states that x != x is true only for NAN values. */
	zassert_true(i != i, NULL);
...
ZTEST(zsl_tests, test_phy_sound_shock_wave_angle)
{
	...
	zassert_true(rc == 0, NULL);
	zassert_true(val_is_equal(theta, 0.6413745117, ZSL_CONSTANT(1E-6)), NULL);
...
ZTEST(zsl_tests, test_phy_sound_dop_effect)
{
	...
	zassert_true(rc == 0, NULL);
	zassert_true(val_is_equal(fl, 854.85, ZSL_CONSTANT(1E-6)), NULL);
...
ZTEST(zsl_tests, test_phy_sound_beat)
{
	...
	zassert_true(rc == 0, NULL);
	zassert_true(val_is_equal(f, 22.5, ZSL_CONSTANT(1E-6)), NULL);
	
	/* Example for 'fa' < 'fb'. */
	rc = zsl_phy_sound_beat(563.8, 697.9, &f);
	zassert_true(rc == 0, NULL);
#ifdef CONFIG_ZSL_SINGLE_PRECISION
	zassert_true(val_is_equal(f, 134.1, ZSL_CONSTANT(1E-4)), NULL);
#else
	zassert_true(val_is_equal(f, 134.1, ZSL_CONSTANT(1E-8)), NULL);
#endif
