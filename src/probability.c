...
zsl_real_t zsl_prob_erf_inv(zsl_real_t *x)
{
	...
	t = ZSL_LOG(t);
	if (ZSL_ABS(t) > 6.125) {
		p = ZSL_CONSTANT(3.03697567e-10);                     //  0x1.4deb44p-32
		p = ZSL_FMA(p, t,  ZSL_CONSTANT(2.93243101e-8));      //  0x1.f7c9aep-26
		p = ZSL_FMA(p, t,  ZSL_CONSTANT(1.22150334e-6));      //  0x1.47e512p-20
		p = ZSL_FMA(p, t,  ZSL_CONSTANT(2.84108955e-5));      //  0x1.dca7dep-16
		p = ZSL_FMA(p, t,  ZSL_CONSTANT(3.93552968e-4));      //  0x1.9cab92p-12
		p = ZSL_FMA(p, t,  ZSL_CONSTANT(3.02698812e-3));      //  0x1.8cc0dep-9
		p = ZSL_FMA(p, t,  ZSL_CONSTANT(4.83185798e-3));      //  0x1.3ca920p-8
		p = ZSL_FMA(p, t, ZSL_CONSTANT(-2.64646143e-1));      // -0x1.0eff66p-2
		p = ZSL_FMA(p, t,  ZSL_CONSTANT(8.40016484e-1));      //  0x1.ae16a4p-1
	} else {                                        // max ulp error = 2.35002
		p = ZSL_CONSTANT(5.43877832e-9);                      //  0x1.75c000p-28
		p = ZSL_FMA(p, t,  ZSL_CONSTANT(1.43285448e-7));      //  0x1.33b402p-23
		p = ZSL_FMA(p, t,  ZSL_CONSTANT(1.22774793e-6));      //  0x1.499232p-20
		p = ZSL_FMA(p, t,  ZSL_CONSTANT(1.12963626e-7));      //  0x1.e52cd2p-24
		p = ZSL_FMA(p, t, ZSL_CONSTANT(-5.61530760e-5));      // -0x1.d70bd0p-15
		p = ZSL_FMA(p, t, ZSL_CONSTANT(-1.47697632e-4));      // -0x1.35be90p-13
		p = ZSL_FMA(p, t,  ZSL_CONSTANT(2.31468678e-3));      //  0x1.2f6400p-9
		p = ZSL_FMA(p, t,  ZSL_CONSTANT(1.15392581e-2));      //  0x1.7a1e50p-7
		p = ZSL_FMA(p, t, ZSL_CONSTANT(-2.32015476e-1));      // -0x1.db2aeep-3
		p = ZSL_FMA(p, t,  ZSL_CONSTANT(8.86226892e-1));
	}
	r = *x * p;
	return r;
}
...
int zsl_prob_entropy(struct zsl_vec *v, zsl_real_t *h)
{
	...
	if (ZSL_ABS(sum) > ZSL_CONSTANT(1E-6)) {
		return -EINVAL;
	}
#endif
